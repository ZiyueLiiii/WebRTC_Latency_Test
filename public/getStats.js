...
var getStats = function(mediaStreamTrack, callback, interval) {
    ...
    function getStatsLooper() {
        getStatsWrapper(function(results) {
            ...
            if (getStatsResult.audio && getStatsResult.video) {
                getStatsResult.bandwidth.RTT = getStatsResult.results.roundTripTime;
                ...
                getStatsResult.bandwidth.totalTPSent = (getStatsResult.audio.bytesSent - getStatsResult.bandwidth.helper.audioBytesSent) + (getStatsResult.video.bytesSent - getStatsResult.bandwidth.helper.videoBytesSent);
                getStatsResult.bandwidth.audioTPSent = getStatsResult.audio.bytesSent - getStatsResult.bandwidth.helper.audioBytesSent;
                getStatsResult.bandwidth.videoTPSent = getStatsResult.video.bytesSent - getStatsResult.bandwidth.helper.videoBytesSent;
    ...
    getStatsParser.checkAudioTracks = function(result) {
        ...
        if (!!result.bytesSent) {
            ...
            getStatsResult.internal.audio[sendrecvType].testPreBytesSent = getStatsResult.internal.audio[sendrecvType].prevBytesSent;
            getStatsResult.internal.audio[sendrecvType].prevBytesSent = result.bytesSent;

            //1 byte = 8 bits, 1 kilobyte (KB) = 8 kilobits (Kb)
            kilobytes = bytes / 1024;
            getStatsResult.audio[sendrecvType].availableBandwidth = bytes.toFixed(1);//bytes.toFixed(1);
            getStatsResult.audio.bytesSent = bytes.toFixed(1);//bytes.toFixed(1);
        }
        ...
        if (!!result.bytesReceived) {
            ...
            // getStatsResult.audio[sendrecvType].availableBandwidth = bytes.toFixed(1);
            getStatsResult.audio.bytesReceived = bytes.toFixed(1);
        }
    ...
    getStatsParser.checkVideoTracks = function(result) {
        ...
        if (!!result.bytesSent) {
            ...
            getStatsResult.internal.video[sendrecvType].testPreBytesSent = getStatsResult.internal.video[sendrecvType].prevBytesSent;
            getStatsResult.internal.video[sendrecvType].prevBytesSent = result.bytesSent;

            kilobytes = bytes / 1024;

            // toFixed(1) is to round the value to one decimal place 
            getStatsResult.video[sendrecvType].availableBandwidth = bytes.toFixed(1);
            getStatsResult.video.bytesSent = bytes.toFixed(1); // the unit is KB
        }
        ...
        if (!!result.bytesReceived) {
            ...
            kilobytes = bytes / 1024;
            // getStatsResult.video[sendrecvType].availableBandwidth = bytes.toFixed(1);
            getStatsResult.video.bytesReceived = bytes.toFixed(1);
        }
        ...
        // calculate latency
        if (!!result.googTargetDelayMs) {//googCurrentDelayMs
            var kilobytes = 0;
            if (!getStatsResult.internal.video.prevGoogCurrentDelayMs) {
                getStatsResult.internal.video.prevGoogCurrentDelayMs = result.googTargetDelayMs;
            }

            var bytes = result.googTargetDelayMs - getStatsResult.internal.video.prevGoogCurrentDelayMs;
            getStatsResult.internal.video.prevGoogCurrentDelayMs = result.googTargetDelayMs;
    ...
    getStatsParser.candidatePair = function(result) {
        ...
        if (isSafari) {
            ...
            if (!!result.bytesSent) {
                ...
                getStatsResult.video[sendrecvType].availableBandwidth = bytes.toFixed(1);
                getStatsResult.video.bytesSent = bytes.toFixed(1);
            }
            ...
            if (!!result.availableOutgoingBitrate) {
                ...
                kilobytes = bytes / 1024;
                // getStatsResult.video[sendrecvType].availableBandwidth = bytes.toFixed(1);
                getStatsResult.video.availableOutgoingBitrate = bytes.toFixed(1);
            }
            ...
            if (!!result.availableIncomingBitrate) {
                ...
                kilobytes = bytes / 1024;
                // getStatsResult.video[sendrecvType].availableBandwidth = bytes.toFixed(1);
                getStatsResult.video.availableIncomingBitrate = bytes.toFixed(1);
            }
        }
    };
    ...
    getStatsParser.inboundrtp = function(result) {
        ...
        if (!!result.bytesSent) {
            ...
            getStatsResult[mediaType][sendrecvType].availableBandwidth = bytes.toFixed(1);
            getStatsResult[mediaType].bytesSent = bytes.toFixed(1);
        }
        ...
        if (!!result.bytesReceived) {
            ...
            kilobytes = bytes / 1024;
            // getStatsResult[mediaType][sendrecvType].availableBandwidth = bytes.toFixed(1);
            getStatsResult[mediaType].bytesReceived = bytes.toFixed(1);
        }
    };
